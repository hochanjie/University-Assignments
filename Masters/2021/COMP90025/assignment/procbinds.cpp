...
int **new2d (int width, int height) {
	...
	dp[0] = dp0;
  int i;
  #pragma omp parallel for shared(height, dp0, dp) private(i)
	for (i = 1; i < width; i++) {
    dp[i] = dp0 + (i * height);
  }
	return dp;
...
int getMinimumPenalty(std::string x, std::string y, int pxy, int pgap, int *xans, int *yans)
{
  ...
  // Concept taken from Tutorial 4 DAG
  ...
  for (d = 0; d < diag; ++d) {
    ...
    #pragma omp parallel for schedule(dynamic) num_threads(2) proc_bind(primary) shared(i,j,dp,length,di,dj,m,n,pgap,pxy) private(iii, jjj, ii, imax, jmax, jj, tile) 
  ...
  #pragma omp parallel 
  {
    ...
    for (int ii = i; ii > 0; ii--) {
      xans[ii + xx] = (int)x[ii - 1];
    }

    #pragma omp for nowait
    for (int x_pos2 = xx; x_pos2 > 0; x_pos2--) {
      xans[x_pos2] = (int)'_';
    }

    #pragma omp for nowait
    for (int jj = j; jj > 0; jj--) {
      yans[jj + yy] = (int)y[jj - 1];
    }

    #pragma omp for nowait
    for (int y_pos2 = yy; y_pos2 > 0; y_pos2--) {
      yans[y_pos2] = (int)'_';
    }
  }
	int ret = dp[m][n];
